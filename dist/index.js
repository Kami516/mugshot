...
try {
    ...
    (0, canvas_1.registerFont)(path.join(__dirname, 'fonts', 'Impact.ttf'), { family: 'Impact' });
    (0, canvas_1.registerFont)(path.join(__dirname, 'fonts', 'Arial-Bold.ttf'), { family: 'Arial-Bold' });
    (0, canvas_1.registerFont)(path.join(__dirname, 'fonts', 'AntonSC.ttf'), { family: 'AntonSC' });
    (0, canvas_1.registerFont)(path.join(__dirname, 'fonts', 'GeistMono.ttf'), { family: 'GeistMono' });
}
catch (error) {
...
// Help command
bot.help((ctx) => {
    ctx.reply('This bot creates a stylized trading card with your trade details. 📈\n\n' +
        'Commands:\n' +
...
bot.on((0, filters_1.message)('text'), async (ctx) => {
    ...
    if (!userId || !typedContext.session)
        return;
    ...
    switch (session.step) {
        ...
        case 'chainChoice':
            ...
            try {
                ...
                const cardBuffer = await generateTradingCard(session.ticker, session.initialInvestment, session.finalAmount, price, session.chain);
...
async function generateTradingCard(ticker, initialInvestment, finalAmount, tokenPrice = 150, chain = 'SOL') {
    ...
    try {
        ...
        if (fs.existsSync(bgImagePath)) {
            const bgImage = await (0, canvas_1.loadImage)(bgImagePath);
    ...
    try {
        ...
        if (fs.existsSync(tokenLogoPath)) {
            const tokenLogo = await (0, canvas_1.loadImage)(tokenLogoPath);
            if (chain === 'ETH') {
        ...
        if (fs.existsSync(tokenLogoPath)) {
            ...
            try {
                ...
                if (fs.existsSync(strokePath)) {
                    const strokeImage = await (0, canvas_1.loadImage)(strokePath);
                    // stroke.png position
